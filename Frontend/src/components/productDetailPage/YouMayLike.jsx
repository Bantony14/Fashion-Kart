...
const product =[
  {
    ...
    "isTrending": true,
    "isNew": false,
    "description": "Premium oversized cotton t-shirt with bold front graphic print.",
    "fabric": "100% Cotton",
    "fit": "Oversized Fit",
...
<h2 className="text-xl font-semibold mb-6">You May Also Like</h2>
        <div className="flex gap-6 overflow-x-auto pb-2">
          {product.map((item) => (
            <div key={item} className="min-w-[190px] bg-white rounded-lg border p-3 hover:shadow">
              <div className="bg-gray-100 p-4 rounded">

              
                <img 
                src={item.image}
                className="h-36 object-contain mx-auto cursor-pointer " 
                onClick={()=>dispatch(addProductDetail({id : item.id , src : item.image , name : item.title , price :item.price , detail : item.description , oldPrice : item.originalPrice}))}
                />
